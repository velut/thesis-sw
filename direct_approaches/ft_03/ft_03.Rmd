---
title: "Former Task 03 (Context Set)"
...
This notebook and the contained analysis process were initially developed to analyze
our direct tasks, in which 1:N comparisons are performed.

The former tasks, presented in the "Activity Matching with Human Intelligence" paper,
share some similarities with our tasks, especially the tasks "Label Only" and "Context One".

Thus, it was possible to adapt these notebooks to also analyze the former tasks.
However, some differences are present with respect to the original implementation,
especially in the analysis of the "Context Set" task.
...
# Path of the task output files compiled by Crowdflower.
# These files should be present inside the 'input' folder.
CF_TASK_FILE_82_83_PATH <- file.path(INPUT_PATH, "FT_03__82_83__f882868.csv")
CF_TASK_FILE_82_84_PATH <- file.path(INPUT_PATH, "FT_03__82_84__f882870.csv")
CF_TASK_FILE_83_89_PATH <- file.path(INPUT_PATH, "FT_03__83_89__f884528.csv")

# Since the output files of this task are missing the
# activity IDs required for the analysis, we must
# read them from another file.
ORIGINAL_NODE_ATTRS_FILE_PATH <- file.path(INPUT_PATH, "original_node_attributes.csv")
...
```{r}
# Get the tibble representing the activity attributes
# (e.g., IDs, labels)
get_node_attrs <- function() {
  read_csv(
    ORIGINAL_NODE_ATTRS_FILE_PATH,
    # Parse all three columns as character
    col_types = "ccc"
  ) %>% 
    mutate(
      # Remove the apostrophe from labels as task output data
      # does not contain it (e.g., "bachelor's" => "bachelors")
      node_label = str_replace(node_label, "\'", "")
    )
}
```
...
get_task_output <- function() {
  # Differently from other tasks, the output data
  # of this task consists in three different files.
  # Thus, we need to read them 
  # and merge them into a single tbl.
  
  data_raw_82_83 <- read_csv(CF_TASK_FILE_82_83_PATH)

  data_raw_82_84 <- 
    read_csv(CF_TASK_FILE_82_84_PATH) %>% 
    # Cast to character (common type of the three raw inputs) 
    mutate(
      processa = as.character(processa),
      processb = as.character(processb)
    )

  data_raw_83_89 <- read_csv(CF_TASK_FILE_83_89_PATH)
  ...
  data_raw %>%
    # In former tasks, test questions were used.
    # Thus, the full report from Crowdflower
    # also contains the answers to
    # these questions, which are not
    # relevant to our analysis.
    # These rows should be discarded
    # and only the actual task rows
    # (where golden == "false") should
    # be kept.
    filter(
      str_detect(`_golden`, "false")
    )
}
```
...
Since the output format of this task contains responses spread over many attributes and
since the activity IDs are not present, we need to gather the responses and reconstruct
from the activity labels the possible original IDs.

However, this operation is not perfect.
In fact, in some cases of shared labels between activities it's impossible
to find the original IDs associated to the homonym activities.
In such cases (actually one), we manually assigned the required IDs.

All the reconstruction operations are encapsulated in the `adapt_data_for_analysis()` function.
```{r}
# Adapt the data to be analyzed to the format expected
# by the analysis process
adapt_data_for_analysis <- function(tbl) {
  # Gather the attributes containing the answers given by workers
  gather_answer_attributes <- function(in_tbl) {
    in_tbl %>%
    # Gather left activities 1-4 into a single column
    gather(
      a1:a4,
      key = "type_a",
      value = "node_label_1"
    ) %>% 
    # Gather right activities 1-4 into a single column
    gather(
      b1:b4,
      key = "type_b",
      value = "node_label_2"
    ) %>% 
    # Gather relationships 1-4 into a single column
    gather(
      r1:r4,
      key = "type_r",
      value = "pair_relationship"
    ) %>% 
    # Gather similarity scores 1-4 into a single column
    gather(
      sim1:sim4,
      key = "type_sim",
      value = "fragment_relevance"
    ) %>% 
    # After gathering, we have four new attribute columns:
    # two for activities labels, one for relationships,
    # and one for similarity scores.
    # We also have four other attribute columns with the original column
    # name of a value (e.g., a value in 'node_label_1' will
    # have an associated value 'aX' in column 'type_a').
    # For more information, see dplyr's 'gather()' documentation.
    #
    # Mutate all the 'type_*' attributes
    # by removing prefixes and keeping only
    # the number of the configuration
    # (e.g., "a1" => "1"; "sim4" => "4").
    mutate_at(
      # If attribute column name starts with "type_"
      vars(starts_with("type_")),
      # replace cell contents by removing prefixes
      funs(str_replace(., "a|b|r|sim", ""))
    ) %>% 
    # Gathering multiple attributes introduces many rows
    # which are not significant to our analysis.
    # From all the existing rows, we only need to keep the ones
    # that represent the real responses of workers.
    # In other words, we need to keep the rows
    # that share the same "type configuration"
    # (e.g., a row containing 'a1' with 'b1' with 'r1' with 'sim1').
    # After this step, we should have a number of rows that is
    # four times the number of rows when attributes are
    # not gathered.
    filter(
      type_a == type_b,
      type_b == type_r,
    ...
    # Now remove 'type_*' attributes as they 
  ...
  # Add node ids (missing from raw data) 
  # based on the activities' labels.
  introduce_node_ids <- function(in_tbl) {
    # Support tibble with labels and IDs
    orig_node_attrs <- get_node_attrs()
    
    in_tbl %>%
    # Add activity IDs missing from raw output data.
    # First for left activities ('node_id_1')
    left_join(
      orig_node_attrs,
      by = c(
        "proc_id_1" = "process_id",
        "node_label_1" = "node_label"
      )
    ) %>%
    rename(
      node_id_1 = node_id
    ) %>%
    # then for right activities ('node_id_2')
    left_join(
      orig_node_attrs,
      by = c(
        "proc_id_2" = "process_id",
        "node_label_2" = "node_label"
      )
    ) %>%
    rename(
      node_id_2 = node_id
    ) %>%
    # Set to NA wrong node IDs coming from different
    # activities with the same label.
    # Activities with IDs that are unset here 
    # will be removed later.
    # This was checked manually.
    mutate(
      # Fix "certificate received" from process 83
      # in 'node_id_1' column
      node_id_1 = if_else(
        (node_id_1 == "2700" & str_detect(img_1, "p83-f5")) |
          (node_id_1 == "2702" & str_detect(img_1, "p83-f4")),
        NA_character_,
        node_id_1
      ),
      # Fix "certificate received" from process 83
      # in 'node_id_2' column
      node_id_2 = if_else(
        (node_id_2 == "2700" & str_detect(img_2, "p83-f5")) |
          (node_id_2 == "2702" & str_detect(img_2, "p83-f4")),
        NA_character_,
        node_id_2
      ),
      # Fix "documents received" from process 89
      # in 'node_id_2' column
      node_id_2 = if_else(
        (node_id_2 == "2884" & str_detect(img_2, "p89-f2")) |
          (node_id_2 == "2899" & str_detect(img_2, "p89-f1")),
        NA_character_,
        node_id_2
      ),
      # Fix "rejected" from process 89.
      # We don't have enough data to establish
      # which "rejected" activity was selected
      # as both activities are present in the same
      # fragment image.
      # We decided to keep the right choice here.
      node_id_2 = if_else(
        unit_id == "904940181" & node_id_2 == "2883",
        NA_character_,
        node_id_2
      )
    ) %>%
    # Replace NAs in 'node_id_*' attributes with "NONE".
    # This is necessary because some rows contain "None"
    # as the activity label to indicate that no activity
    # was selected.
    # In the previously performed join,
    # since "None" is not a valid labels we get NAs.
    mutate_at(
      vars(starts_with("node_id_")),
      funs(str_replace_na(., replacement = "NONE"))
    )
  }

  # From all the rows available keep only
  # those representing analyzable pairs.
  keep_only_valid_pairs <- function(in_tbl) {
    in_tbl %>%
    filter(
      # Keep only pairs that are valid
      node_id_1 != "NONE" & node_id_2 != "NONE",
      # Keep only judgments where worker said "Yes"
      is_pair == TRUE,
      # Keep only judgments where fragment relevance was expressed.
      # Judgment '1921209345' is the only one removed by this predicate.
      !is.na(fragment_relevance)
    ) 
  }

  tmp <-
    tbl %>%
    gather_answer_attributes() %>%
    introduce_node_ids() %>%
    keep_only_valid_pairs()
}
```
...
#
# In this task, for each suggested activity pair
# workers would add a relevance score.
# In other words, instead of a single general score
# one to four (possibly ten) scores could be submitted
# by a single worker in each unit.
# To compute the MFR in this case,
# we choose to consider all the distinct activity pairs
# identified by different workers and average their
# associated scores.
...
```{r}
# In this task, this step is not required
...
add_paired_attr <- function(tbl) {
  tmp <- 
    ...
    mutate(
      ...
      # Pairs suggested as "part of"
      paired_part_of = str_detect(pair_relationship, "part of"),
...
join_tbl_with_gold <- function(tbl) {
  ...
  tmp <- 
    ...
    mutate(
      # In this task, workers were asked to select activities based
...
data_base <- 
  ...
  select(
    ...
    # Unfortunately activity IDs were not
    # preserved in the original task design.
    # Thus, we will have to reintroduce them later.
    proc_id_1 = processa,
    proc_id_2 = processb,
    # Design questions
    # (activity 'a1' is in relationship 'r1'
    # with activity 'b1' and
    # their similarity score is 'sim1').
    # At most out of 10 available slots only the first
    # four were filled, thus we select only those 
    # (implicitly discarding slots 5-10).
    #
    # Labels (NOT IDs) of selected activities
    a1, a2, a3, a4,
    b1, b2, b3, b4,
    ...
    # Similarity scores
    sim1, sim2, sim3, sim4,
    # Images of fragments from where the
    # activities were picked
    img_1 = urla,
    img_2 = urlb,
    # Did the worker find any pair between the proposed fragments?
    is_pair = 
      are_there_any_reasonable_activity_matches_in_the_two_process_fragments_shown_above
  )

# Add 'is_pair' attribute and fix process IDs
data_base <-
  data_base %>%
  mutate(
    # Change 'is_pair' to TRUE/FALSE
    is_pair = str_detect(is_pair, "Yes"),
    # Remove "p" prefix from processes ids
    # (e.g., "p82" => "82")
    proc_id_1 = str_replace(proc_id_1, "p", ""),
    proc_id_2 = str_replace(proc_id_2, "p", "")
  )
...
# Note: In this task inconsistent judgments,
# as defined for our direct tasks, are not
# possible. However, for compatibility the
# "incjdgTRUE" string in the dataset attribute
# is present. It simply means that no judgments
# were removed.
#
# In this task, we also need to adapt the data
# to conform to analysis' requirements.
data_all <-   
  data_3jdg_all %>%
  adapt_data_for_analysis()
```
