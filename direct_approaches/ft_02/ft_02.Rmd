---
title: "Former Task 02 (Context One)"
...
CF_TASK_FILE_PATH <- file.path(INPUT_PATH, "FT_02__f885200.csv")
...
add_paired_attr <- function(tbl) {
  tmp <- 
    ...
    mutate(
      ...
      paired_part_of = str_detect(pair_relationship, "partof"),
...
data_base <- 
  ...
  select(
    ...
    # First process
    proc_id_1 = processid_a,
    node_id_1 = nodeid_a,
    node_label_1 = nodelabel_a,
    # Second process
    proc_id_2 = processid_b,
    # This former task presented 1:1 activity pairs
    # thus 'node_ids_2' contains always a single node id.
    # Similarly this happens with 'nodel_labels_2'.
    # We keep the plural name, coming from notebooks 
    # analyzing our direct tasks, to minimize 
    # the differences between notebooks.
    node_ids_2 = nodeid_b,
    node_labels_2 = nodelabel_b,
    # Design questions, change names accordingly
    fragment_relevance =
      how_similar_do_you_think_are_the_two_highlighted_activities,
    # Workers could vote "yes"/"no" to say if the activities presented
    # in a pair are similar or not
    is_pair = 
      are_the_two_activities_highlighted_in_red_similar,
    # Workers could describe the relationship between the two activities
    pair_relationship = 
      if_you_think_the_two_activities_are_similar_how_would_you_describe_their_relationship,
    worker_explanation = 
     please_comment_in_which_sense_these_two_activities_are_similar
  )

# This mutation is needed to adjust pair attributes
# and to correctly setup the 'relevant_node_ids'
# attribute used in the next steps
data_base <-
  data_base %>%
  mutate(
    # Change 'is_pair' to TRUE/FALSE
    is_pair = str_detect(is_pair, "yes"),
    # Add missing values for 'pair_relationship'
    pair_relationship = if_else(
      is.na(pair_relationship),
      "NONE",
      pair_relationship
    ),
    # Add the 'relevant_node_ids' attribute (necessary for analysis)
    # using the following strategy.
    # If the worker said "yes" (i.e., he identified a pair),
    # copy 'node_ids_2' (which contains the id from 'nodeid_b')
    # into 'relevant_node_ids' ,
    # otherwise insert "NONE".
    relevant_node_ids = if_else(
      is_pair,
      as.character(node_ids_2),
      "NONE"
    )
  )
